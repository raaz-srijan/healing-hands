             ...
             <div className="space-y-6 animate-fade-in-up">
                <div className="flex flex-col md:flex-row justify-between md:items-center gap-4">
                    <div>
                        ...
                        <p className="text-sm text-gray-500">Access and download patient medical reports.</p>
                    </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {reports.map((report) => (
                        <div key={report.id} className="bg-white p-5 rounded-xl shadow-sm border border-gray-100 hover:shadow-md transition-shadow group">
                            <div className="flex justify-between items-start mb-4">
                                <div className={`p-3 rounded-lg ${report.type === 'Lab Report' ? 'bg-blue-50 text-blue-600' : 'bg-orange-50 text-orange-600'}`}>
                                    <FaFileMedical className="text-xl" />
                                </div>
                                <span className={`px-2 py-1 rounded text-xs font-semibold ${report.status === 'Ready' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}>
                                    {report.status}
                                </span>
                            </div>
                            
                            <h3 className="font-bold text-gray-800 mb-1">{report.text}</h3>
                            <p className="text-sm text-gray-500 mb-4">Patient: <span className="text-gray-700 font-medium">{report.patient}</span></p>
                            ...
                            <div className="pt-4 border-t border-gray-50 flex justify-between items-center text-sm">
                                ...
                                <div className="flex gap-2">
                                    <button className="p-2 text-gray-400 hover:text-sky-600 transition-colors" title="View"><FaEye /></button>
                                    <button className="p-2 text-gray-400 hover:text-sky-600 transition-colors" title="Download"><FaDownload /></button>
                                </div>
                            </div>
                        </div>
                    ))}
